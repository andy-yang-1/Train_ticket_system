...
string IntToSTRING( int temp_int ){
    string temp_ans = "" ;
    if ( temp_int == 0 ){
        temp_ans = "0" ;
        return temp_ans ;
    ...
    while ( temp_int > 0 ){
        temp_ans = (char) ( temp_int % 10 + '0' ) + temp_ans ;
        temp_int /= 10 ;
    }
    return temp_ans ;
}
...
void date::print_date()
{
    if ( all_time == 0 ){
//        cout << "xx-xx xx:xx" ;
        front_end_answer.append("xx-xx xx:xx") ;
        return ;
    }
...
//    cout << temp_minute ;
    string temp_str ;
    temp_str = "0" + IntToSTRING(temp_month) + "-" ;
    if ( temp_day < 10 ) temp_str += '0' ;
    temp_str += IntToSTRING(temp_day) + " " ;
    if ( temp_hour < 10 ) temp_str += '0' ;
    temp_str += IntToSTRING(temp_hour) + ":" ;
    if ( temp_minute < 10 ) temp_str += '0' ;
    temp_str += IntToSTRING(temp_minute) ;
    front_end_answer.append(temp_str) ;
}
...
//    cout << user_name << " " << chinese_name << " " << mailAddr << " " << privilege << endl ;
    front_end_answer.append(user_name + string(" ") + chinese_name + string(" ") + mailAddr + string(" ") + IntToSTRING(privilege) + "\n" ) ;
}
...
//    cout << trainID << " " << all_station[location_1] << " " ;
    front_end_answer.append(trainID + string(" ") + all_station[location_1] + string(" ") ) ;
    set_off_date.print_date() ;
//    cout << " -> " << all_station[location_2] << " " ;
    front_end_answer.append(string(" -> ") + all_station[location_2] + string(" ") ) ;
    arrive_in_date.print_date() ;
//    cout << " " << all_price[location_2] - all_price[location_1] ;
    front_end_answer.append(string(" ")+ IntToSTRING(all_price[location_2]-all_price[location_1])) ;
}
...
//    cout << trainID << " " << train_type << endl ;
    front_end_answer.append(string(trainID) + string(" ") + train_type + string("\n") ) ;
//    cout << all_station[1] << " " ;
    front_end_answer.append(all_station[1]+string(" ")) ;
    err_date.print_date() ;
    for ( int i = 2 ; i <= station_num ; i++ ){
//        cout << " -> " ;
        front_end_answer.append(" -> ") ;
        query_day.print_date() ;
//        cout << " " << all_price[i-1] << " " << all_seat[index_day][i-1] << endl ;
        front_end_answer.append(string(" ") + IntToSTRING(all_price[i-1]) + string(" ") + IntToSTRING(all_seat[index_day][i-1]) + string("\n") ) ;
//        cout << all_station[i] << " " ;
        front_end_answer.append(all_station[i]+string(" ")) ;
...
//    cout << " " << all_price[station_num] << " x" << endl ;
    front_end_answer.append(string(" ") + IntToSTRING(all_price[station_num]) + " x\n" ) ;
}
...
//    }
    switch (deal_status) {
        case succeed: front_end_answer.append("[success] ") ; break ;
        case pending: front_end_answer.append("[pending] ") ; break ;
        case refunded: front_end_answer.append("[refunded] ") ; break ;
    }
...
//    cout << " " << price << " " << ticket_num << endl ;
    front_end_answer.append(string(" ") + IntToSTRING(price) + string(" ") + IntToSTRING(ticket_num) + string("\n") ) ;
}
